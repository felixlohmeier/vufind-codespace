...
# ── 4. Install VuFind DEB (first pass may fail with unresolved dependencies) ──
echo "--- Installing VuFind DEB ---"
sudo dpkg -i /tmp/vufind.deb || true
sudo apt-get install -f -y -q

# ── 5. Run VuFind installer to create local configuration files ────────────────
...
# ── 7. Link Apache configuration (DEB may already have done this) ─────────────
...
# ── 8. Write environment variables to profile.d ───────────────────────────────
...
# ── 9. Start MariaDB and set up database ──────────────────────────────────────
...
# ── 10. Configure VuFind database connection in config.ini ────────────────────
